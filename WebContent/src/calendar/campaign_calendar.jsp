...
<script type="text/javascript" charset="utf-8">
	...
	function init() {
		...
		if(campaignid && campaignid!=null && campaignid.length==32){
			url=www_url+"/rest/appcalendar/campaign?token="+token+"&campaignid="+campaignid;
		}else{
			url=www_url+"/rest/appcalendar/campaign?token="+token
		}
...
-->
	...
	<div style="position: relative; height:95px;background-color:#3D7F1C;border-bottom:5px solid #828282;">
		...
		<div id="contbox" style="position: relative; padding: 22px 25px 0 75px; font: normal 17px Arial, Helvetica; color:white;">
			<div style="position: absolute; left: 75px; top: 22px; border-right:5px solid #2D8EB6;color:#2D8EB6;width:175px;height:50px;text-align:right;padding-right:25px;">Campaign Calendar</div>
			<div style="padding-left: 205px; min-width: 400px;">
                <div style="font-size:12px;padding-left:20px;">Browse the events in Day, Week, Month, Year, or Agenda Views with All Campaigns.</div>
    		</div>
		</div>
		<a style="position: absolute; right: 25px; top: 22px;" href="javascript:showTravelCalendar(null,null);"><img src="images/btn-right.gif"></a>
	</div>
	<!-- end. info block -->
    <ul>
        <li>
