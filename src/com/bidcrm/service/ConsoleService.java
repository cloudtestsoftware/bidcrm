		...
		import com.bidcrm.dao.ConsoleDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class ConsoleService {
			...
			// Get record count with filter for Console
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getConsoleRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new ConsoleDao(uriInfo,header).getConsoleRecordCount();
				} catch (AuthenticationException e) {
			...
			// Get all Consolelog rows against object ID for Console
			@GET
			@Path("/{id}/consolelog")
			@Produces({"application/xml"})
			public Rows getConsolelogRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new ConsoleDao(uriInfo,header).getConsolelogRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
