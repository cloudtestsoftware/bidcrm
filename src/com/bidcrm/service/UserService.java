		...
		*/
		...
		public class UserService {
			...
			public Items getUserForm() {
				Items items = null;
				try {
					items=new UserDao(uriInfo,header).getUserForm();
				} catch (AuthenticationException e) {
			...
			// Get record count with filter for User
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getUserRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new UserDao(uriInfo,header).getUserRecordCount();
				} catch (AuthenticationException e) {
