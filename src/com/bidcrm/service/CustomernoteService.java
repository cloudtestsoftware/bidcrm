		...
		import com.bidcrm.dao.CustomernoteDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class CustomernoteService {
			...
			// Get record count with filter for Customernote
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getCustomernoteRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new CustomernoteDao(uriInfo,header).getCustomernoteRecordCount();
				} catch (AuthenticationException e) {
			...
			public Rows postFormDataCustomernote(@Context UriInfo uriInfo,@Context  HttpHeaders header,@FormDataParam("body") String xml) {
				...
				try {
					post=new CustomernoteDao(uriInfo,header);
					post.setPostXml(xml.trim());
					if(post.postCustomernoteContainer()){
						rows=post.getCustomernoteRowModified();
					}else{
