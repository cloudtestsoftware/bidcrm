		...
		import com.bidcrm.dao.CompanyDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class CompanyService {
			...
			// Get record count with filter for Company
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getCompanyRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new CompanyDao(uriInfo,header).getCompanyRecordCount();
				} catch (AuthenticationException e) {
			...
			// Get all Territory rows against object ID for Company
			@GET
			@Path("/{id}/territory")
			@Produces({"application/xml"})
			public Rows getTerritoryRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new CompanyDao(uriInfo,header).getTerritoryRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
