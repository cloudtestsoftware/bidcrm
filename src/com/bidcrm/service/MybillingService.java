		...
		import com.bidcrm.dao.MybillingDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class MybillingService {
			...
			public Items getMybillingForm() {
				Items items = null;
				try {
					items=new MybillingDao(uriInfo,header).getMybillingForm();
				} catch (AuthenticationException e) {
			...
			// Get record count with filter for Mybilling
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getMybillingRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new MybillingDao(uriInfo,header).getMybillingRecordCount();
				} catch (AuthenticationException e) {
			...
			public Rows postFormDataMybilling(@Context UriInfo uriInfo,@Context  HttpHeaders header,@FormDataParam("body") String xml) {
				...
				try {
					post=new MybillingDao(uriInfo,header);
					post.setPostXml(xml.trim());
					if(post.postMybillingContainer()){
						rows=post.getMybillingRowModified();
					}else{
