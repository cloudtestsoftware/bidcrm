		...
		import com.bidcrm.dao.PartlistDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class PartlistService {
			...
			public Items getPartlistForm() {
				Items items = null;
				try {
					items=new PartlistDao(uriInfo,header).getPartlistForm();
				} catch (AuthenticationException e) {
			...
			// Get record count with filter for Partlist
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getPartlistRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new PartlistDao(uriInfo,header).getPartlistRecordCount();
				} catch (AuthenticationException e) {
			...
			// Get all Parts rows against object ID for Partlist
			@GET
			@Path("/{id}/parts")
			@Produces({"application/xml"})
			public Rows getPartsRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new PartlistDao(uriInfo,header).getPartsRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
			...
			// Get all Storepartlist rows against object ID for Partlist
			@GET
			...
			public Rows getStorepartlistRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new PartlistDao(uriInfo,header).getStorepartlistRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
