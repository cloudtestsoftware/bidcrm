		...
		import com.bidcrm.dao.EmployeeDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class EmployeeService {
			...
			public Items getEmployeeForm() {
				Items items = null;
				try {
					items=new EmployeeDao(uriInfo,header).getEmployeeForm();
				} catch (AuthenticationException e) {
			...
			// Get record count with filter for Employee
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getEmployeeRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new EmployeeDao(uriInfo,header).getEmployeeRecordCount();
				} catch (AuthenticationException e) {
			...
			// Get all Reviewer rows against object ID for Employee
			@GET
			@Path("/{id}/reviewer")
			@Produces({"application/xml"})
			public Rows getReviewerRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new EmployeeDao(uriInfo,header).getReviewerRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
