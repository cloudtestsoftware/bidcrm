		...
		import com.bidcrm.dao.BizprofileDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class BizprofileService {
			...
			public Items getBizprofileForm() {
				Items items = null;
				try {
					items=new BizprofileDao(uriInfo,header).getBizprofileForm();
				} catch (AuthenticationException e) {
			...
			// Get record count with filter for Bizprofile
			@GET
			@Path("/recordcount")
			@Produces({"application/json"})
			public Response getBizprofileRecordCount() throws JSONException {
				JSONObject rows = new JSONObject();
				try {
					rows=new BizprofileDao(uriInfo,header).getBizprofileRecordCount();
				} catch (AuthenticationException e) {
			...
			public Rows postFormDataBizprofile(@Context UriInfo uriInfo,@Context  HttpHeaders header,@FormDataParam("body") String xml) {
				...
				try {
					post=new BizprofileDao(uriInfo,header);
					post.setPostXml(xml.trim());
					if(post.postBizprofileContainer()){
						rows=post.getBizprofileRowModified();
					}else{
