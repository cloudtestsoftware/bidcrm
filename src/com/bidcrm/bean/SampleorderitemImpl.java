	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class SampleorderitemImpl extends QueryImpl implements QueryEvent {
		 protected static Log logger = LogFactory.getLog(SampleorderitemImpl.class); 
		 ...
		 public String sampleorderitemAccessFilter="";
		  ...
		  public  TemplateTable  doSelect(String[] column,String[] datatype,String parentfilter,boolean isform,String accessfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Sampleorderitem",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Sampleorderitem",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  doSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter,String accessfilter){
			...
			EventListener.registerPreQueryChild("Sampleorderitem",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Sampleorderitem",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   doDelete(String[] childtabs){
			...
			String sql=query.removeSql("Sampleorderitem",this.getParentobjid(),childtabs,groupuser);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Sampleorderitem",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Sampleorderitem",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Sampleorderitem",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Sampleorderitem",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Sampleorderitem",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Sampleorderitem",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
			...
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
					logger.info(usql);
				 tu.applyObjectRule("Sampleorderitem",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Sampleorderitem","Sampleorderitem",getParentobjid());
				 EventListener.registerPostInsertEvent("Sampleorderitem",maindata);
				 return(true);
			}
			 return(false);
		}
