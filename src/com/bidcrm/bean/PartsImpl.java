	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class PartsImpl extends QueryImpl implements QueryEvent {
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					sql=query.makeBulkSQL(true,this.getMainxml(),"Parts2PartList",(QueryImpl)this);
			...
			if(sql==null){
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getQrinfoxml()) ){
					 sql+=query.makeBulkSQL(false,getQrinfoxml(),"QRInfo2Parts",(QueryImpl)this);
				 ...
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"QRInfo","QRInfo2Parts",getParentobjid());
				}
				 if(!tu.isEmptyValue(this.getSamplesxml()) ){
					 sql+=query.makeBulkSQL(false,getSamplesxml(),"Samples2Parts",(QueryImpl)this);
				 ...
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"Samples","Samples2Parts",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
				 ...
				 tu.applyMTMRelation("qrinfo","Parts",getParentobjid());
				 ...
				 tu.applyMTMRelation("samples","Parts",getParentobjid());
