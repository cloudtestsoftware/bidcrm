	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class StoreImpl extends QueryImpl implements QueryEvent {
		 protected static Log logger = LogFactory.getLog(StoreImpl.class); 
		 ...
		 public void setStorepartlistdeleteid(String storepartlistdeleteid) {
		  ...
		  public  TemplateTable  doSelect(String[] column,String[] datatype,String parentfilter,boolean isform,String accessfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Store",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Store",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  doSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter,String accessfilter){
			...
			EventListener.registerPreQueryChild("Store",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Store",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   doDelete(String[] childtabs){
			...
			String sql=query.removeSql("Store",this.getParentobjid(),childtabs,groupuser);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Store",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Store",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Store",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Store",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					sql=query.makeBulkSQL(true,this.getMainxml(),"Store2Site",(QueryImpl)this);
					maindata=query.getTableData();
					tu.applyObjectRule("Store",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Store",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getStorepartlistxml()) ){
					 sql+=query.makeBulkSQL(false,getStorepartlistxml(),"StorePartList2Store",(QueryImpl)this);
				 ...
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"StorePartList","StorePartList2Store",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
					logger.info(usql);
				 tu.applyObjectRule("Store",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Store","Store",getParentobjid());
				 EventListener.registerPostInsertEvent("Store",maindata);
				 tu.applyObjectRule("Storepartlist",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,storepartlistdata);
				 tu.applyMTMRelation("storepartlist","Store",getParentobjid());
