	...
	protected String [] propBidtaskslist={"stage"};
		...
		public Rows getBidtasksSummaryRows(){
			TemplateTable tab=this.doSelect(summarycol,summarysqldatatype,this.BidtasksFilter,false,bidtasksAccessFilter);
		...
		public Items getBidtasksForm(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,this.BidtasksFilter,true,bidtasksAccessFilter);
			Items items=tu.getXMLForm(tab, "Bidtasks",codeBidtaskslist,propBidtaskslist,relationBidtaskslist,maincolcaption,maindatadomain,mainformfields,(QueryImpl)this);
			return items;
		}

		public Rows getBidtasksRows(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,null,false,bidtasksAccessFilter);
			Rows rows=tu.getXMLRows(tab, "Bidtasks",codeBidtaskslist,propBidtaskslist,relationBidtaskslist,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
		...
		public Rows getBidtasksRowModified(){
			Rows rows=tu.getXMLRows(maindata, "Bidtasks",codeBidtaskslist,propBidtaskslist,relationBidtaskslist,maincolcaption,maindatadomain,(QueryImpl)this);
			return rows;
		}
		...
		public boolean postBidtasksContainer() throws DaoException{
		...
		public Rows getBidtasksByFilter(){
			...
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("bidtasks2")<0){
			...
			Rows rows=tu.getXMLFilterRows(tab, "Bidtasks",codeBidtaskslist,propBidtaskslist,relationBidtaskslist,maincol,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
			Userdata data1= new Userdata("tabs",Arrays.asList(childtabs));
			Userdata data2= new Userdata("tabnames",Arrays.asList(childtabnames));
			Userdata data3= new Userdata("filters",Arrays.asList(maincolsearch));
		 ...
		 public JSONObject getBidtasksRecordCount() throws JSONException{
			String newfilter=" groupuser='"+this.getGroupuser()+"'";
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("bidtasks2")<0){
				newfilter+=" and bidtasks2"+this.getFilters();
			}else{
