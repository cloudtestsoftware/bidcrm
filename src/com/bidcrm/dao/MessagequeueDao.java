	...
	public class MessagequeueDao extends MessagequeueImpl {
		...
		protected String []deletetabs={"messagequeue,"};
		...
		protected String [] maindatadomain={"Raw_t","Raw_t","Name_t","Name_t","Name_t","Code_t","Email_t","Status_t","Name_t"};
		...
		protected String [] searchcol={"objid","name","firstname","lastname","deptcode","status"};
		protected String [] searchcolcaption={"Id","Name","First Name","Last Name","Dept Code","Status"};
		protected String [] searchcoltype={"integer","text","text","text","select","select"};
		protected String [] searchdatadomain={"Id_t","Name_t","Name_t","Name_t","Code_t","Status_t"};
	...
	protected String [] propMessagequeuelist={"status"};
		...
		public Rows getMessagequeueSummaryRows(){
			TemplateTable tab=this.doSelect(summarycol,summarysqldatatype,this.MessagequeueFilter,false,messagequeueAccessFilter);
		...
		public Items getMessagequeueForm(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,this.MessagequeueFilter,true,messagequeueAccessFilter);
			Items items=tu.getXMLForm(tab, "Messagequeue",codeMessagequeuelist,propMessagequeuelist,relationMessagequeuelist,maincolcaption,maindatadomain,mainformfields,(QueryImpl)this);
			return items;
		}

		public Rows getMessagequeueRows(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,null,false,messagequeueAccessFilter);
			Rows rows=tu.getXMLRows(tab, "Messagequeue",codeMessagequeuelist,propMessagequeuelist,relationMessagequeuelist,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
		...
		public Rows getMessagequeueRowModified(){
			Rows rows=tu.getXMLRows(maindata, "Messagequeue",codeMessagequeuelist,propMessagequeuelist,relationMessagequeuelist,maincolcaption,maindatadomain,(QueryImpl)this);
			return rows;
		}
		...
		public boolean postMessagequeueContainer() throws DaoException{
		...
		public Rows getMessagequeueByFilter(){
			...
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("messagequeue2")<0){
			...
			Rows rows=tu.getXMLFilterRows(tab, "Messagequeue",codeMessagequeuelist,propMessagequeuelist,relationMessagequeuelist,maincol,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
			Userdata data1= new Userdata("tabs",Arrays.asList(childtabs));
			Userdata data2= new Userdata("tabnames",Arrays.asList(childtabnames));
			Userdata data3= new Userdata("filters",Arrays.asList(maincolsearch));
		 ...
		 public JSONObject getMessagequeueRecordCount() throws JSONException{
			String newfilter=" groupuser='"+this.getGroupuser()+"'";
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("messagequeue2")<0){
				newfilter+=" and messagequeue2"+this.getFilters();
			}else{
