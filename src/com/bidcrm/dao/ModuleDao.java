	...
	public class ModuleDao extends ModuleImpl {
		...
		protected String [] maindatadomain={"Raw_t","Raw_t","Name_t","Int_t"};
	...
	protected String [] propModulelist={"name"};
		...
		protected String [] moduleobjectdatadomain={"Raw_t","Raw_t","Name_t","Name_t","Type_t","Type_t"};
		...
		protected String [] submoduledatadomain={"Raw_t","Name_t","Name_t","Type_t","Int_t"};
		...
		public Rows getModuleobjectRows(){
			TemplateTable tab=this.doSelectChild("moduleobject", "moduleobject2module",this.getParentobjid(),moduleobjectcol,moduleobjectsqldatatype,this.ModuleobjectFilter,moduleobjectAccessFilter);
			...
			Rows rows=tu.getXMLRows(tab, "moduleobject",codeModuleobjectlist,propModuleobjectlist,relationModuleobjectlist,moduleobjectcolcaption,moduleobjectdatadomain,(QueryImpl)this);
		...
		public Rows getSubmoduleRows(){
			TemplateTable tab=this.doSelectChild("submodule", "submodule2module",this.getParentobjid(),submodulecol,submodulesqldatatype,this.SubmoduleFilter,submoduleAccessFilter);
			String [] propSubmodulelist={"name","scope"};
			String [] codeSubmodulelist={};
			String [] relationSubmodulelist={};
			Rows rows=tu.getXMLRows(tab, "submodule",codeSubmodulelist,propSubmodulelist,relationSubmodulelist,submodulecolcaption,submoduledatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
		...
		public Rows getModuleSummaryRows(){
			TemplateTable tab=this.doSelect(summarycol,summarysqldatatype,this.ModuleFilter,false,moduleAccessFilter);
		...
		public Items getModuleForm(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,this.ModuleFilter,true,moduleAccessFilter);
			Items items=tu.getXMLForm(tab, "Module",codeModulelist,propModulelist,relationModulelist,maincolcaption,maindatadomain,mainformfields,(QueryImpl)this);
			return items;
		}

		public Rows getModuleRows(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,null,false,moduleAccessFilter);
			Rows rows=tu.getXMLRows(tab, "Module",codeModulelist,propModulelist,relationModulelist,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
		...
		public Rows getModuleRowModified(){
			Rows rows=tu.getXMLRows(maindata, "Module",codeModulelist,propModulelist,relationModulelist,maincolcaption,maindatadomain,(QueryImpl)this);
			return rows;
		}
		...
		public boolean postModuleContainer() throws DaoException{
		...
		public Rows getModuleByFilter(){
			...
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("module2")<0){
			...
			Rows rows=tu.getXMLFilterRows(tab, "Module",codeModulelist,propModulelist,relationModulelist,maincol,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
			Userdata data1= new Userdata("tabs",Arrays.asList(childtabs));
			Userdata data2= new Userdata("tabnames",Arrays.asList(childtabnames));
			Userdata data3= new Userdata("filters",Arrays.asList(maincolsearch));
		 ...
		 public JSONObject getModuleRecordCount() throws JSONException{
			String newfilter=" groupuser='"+this.getGroupuser()+"'";
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("module2")<0){
				newfilter+=" and module2"+this.getFilters();
			}else{
