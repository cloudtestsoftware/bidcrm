	...
	protected String [] propAssigntolist={};
		protected String [] codeAssigntolist={};
		protected String [] relationAssigntolist={};
		...
		public Rows getAssigntoSummaryRows(){
			TemplateTable tab=this.doSelect(summarycol,summarysqldatatype,this.AssigntoFilter,false,assigntoAccessFilter);
		...
		public Items getAssigntoForm(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,this.AssigntoFilter,true,assigntoAccessFilter);
			Items items=tu.getXMLForm(tab, "Assignto",codeAssigntolist,propAssigntolist,relationAssigntolist,maincolcaption,maindatadomain,mainformfields,(QueryImpl)this);
			return items;
		}

		public Rows getAssigntoRows(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,null,false,assigntoAccessFilter);
			Rows rows=tu.getXMLRows(tab, "Assignto",codeAssigntolist,propAssigntolist,relationAssigntolist,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
		...
		public Rows getAssigntoRowModified(){
			Rows rows=tu.getXMLRows(maindata, "Assignto",codeAssigntolist,propAssigntolist,relationAssigntolist,maincolcaption,maindatadomain,(QueryImpl)this);
			return rows;
		}
		...
		public boolean postAssigntoContainer() throws DaoException{
		...
		public Rows getAssigntoByFilter(){
			...
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("assignto2")<0){
				newfilter+=" and assignto2"+this.getFilters();
			...
			Rows rows=tu.getXMLFilterRows(tab, "Assignto",codeAssigntolist,propAssigntolist,relationAssigntolist,maincol,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
			Userdata data1= new Userdata("tabs",Arrays.asList(childtabs));
			Userdata data2= new Userdata("tabnames",Arrays.asList(childtabnames));
			Userdata data3= new Userdata("filters",Arrays.asList(maincolsearch));
		 ...
		 public JSONObject getAssigntoRecordCount() throws JSONException{
			String newfilter=" groupuser='"+this.getGroupuser()+"'";
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("assignto2")<0){
				newfilter+=" and assignto2"+this.getFilters();
			}else{
