	...
	protected String [] propQrtaskslist={"stage"};
		...
		public Rows getQrtasksSummaryRows(){
			TemplateTable tab=this.doSelect(summarycol,summarysqldatatype,this.QrtasksFilter,false,qrtasksAccessFilter);
		...
		public Items getQrtasksForm(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,this.QrtasksFilter,true,qrtasksAccessFilter);
			Items items=tu.getXMLForm(tab, "Qrtasks",codeQrtaskslist,propQrtaskslist,relationQrtaskslist,maincolcaption,maindatadomain,mainformfields,(QueryImpl)this);
			return items;
		}

		public Rows getQrtasksRows(){
			TemplateTable tab=this.doSelect(maincol,mainsqldatatype,null,false,qrtasksAccessFilter);
			Rows rows=tu.getXMLRows(tab, "Qrtasks",codeQrtaskslist,propQrtaskslist,relationQrtaskslist,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
		...
		public Rows getQrtasksRowModified(){
			Rows rows=tu.getXMLRows(maindata, "Qrtasks",codeQrtaskslist,propQrtaskslist,relationQrtaskslist,maincolcaption,maindatadomain,(QueryImpl)this);
			return rows;
		}
		...
		public boolean postQrtasksContainer() throws DaoException{
		...
		public Rows getQrtasksByFilter(){
			...
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("qrtasks2")<0){
			...
			Rows rows=tu.getXMLFilterRows(tab, "Qrtasks",codeQrtaskslist,propQrtaskslist,relationQrtaskslist,maincol,maincolcaption,maindatadomain,(QueryImpl)this);
			ArrayList<Userdata> userdata=rows.getUserdata();
			Userdata data1= new Userdata("tabs",Arrays.asList(childtabs));
			Userdata data2= new Userdata("tabnames",Arrays.asList(childtabnames));
			Userdata data3= new Userdata("filters",Arrays.asList(maincolsearch));
		 ...
		 public JSONObject getQrtasksRecordCount() throws JSONException{
			String newfilter=" groupuser='"+this.getGroupuser()+"'";
			if(!tu.isEmptyValue(this.getFilters()) &&this.getFilters().toLowerCase().indexOf("qrtasks2")<0){
				newfilter+=" and qrtasks2"+this.getFilters();
			}else{
